---
name: release-build
description: Build a signed release APK of this OnlyShare app for personal use (gradle release build, zipalign, sign with the Android debug keystore). Use when the user asks for a production or release build, or before the `release-install` skill.
---
...
- Build tools live in `~/Android/Sdk/build-tools/<version>/` (`zipalign`, `apksigner`). Use the newest installed
  version
- gradle writes to `~/.gradle`, which the Bash sandbox denies. Run the build with
  `dangerouslyDisableSandbox: true`
- Give log files an **absolute** path, and delete the log afterwards if it was written inside the repository
...
Run everything in the background with an absolute log path (this is slower than a debug build):
