...
- The app draws nothing, so a screenshot only ever shows the share sheet it opened (or the previous app, when
  the clipboard was empty). To check what it actually shared, read the intent out of the log:
  `adb logcat -d -s ActivityTaskManager | rg android.intent.action.SEND`
- To put something on the clipboard from the host: `adb shell am broadcast` cannot do it; use
  `adb shell input` in a text field, or copy in an app on the device
